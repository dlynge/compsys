 ...
 * dlynge
 *
 * This file contains drivers to test the BST package. Note that this 
 * implementation includes a parent node within the node struct. This is null
 * if the node is the root of the tree, or points to the parent node otherwise.
 *
 * The unit driver builds a custom tree and removes selected keys 
 *   -u 0: example test to remove leaves, 12 and 20, then internal nodes
 *   -u 1: (48) is missing its right-left child and (16) is missing its 
 *         left-right child
 *   -u 2: example tests: (16) is missing its right-left child and (48) is 
 *         missing its left-right child
 *   -u 3: example deletion with many children
 *   -u 4: check replace for duplicate key
 *   -u 5: example from homework set
 *   -u 6: example with poor shape that improves wtih balancing 
 *   -u 7: example removing root and removing the new root
 *   -u 8: example replacing root and then removing it
 *   -u 9: example replacing a leaf and removing it
 *
 ...
 * 
 * Assumptions: The user picks one of the options above and their computer
 * is able to allocate enough memory to store the tree.
 * 
 * Bugs: none known.
 *
 */
...
int main(int argc, char **argv)
{
    ...
    if (UnitNumber == 7)
    {
	// example removing root and removing the new root
	const int ins1b[] = {32,16,48,8,24,40,56,4,12,28,36,52,60};
	const int del1b[] = {32, 36}; 
        unitDriver(ins1b, sizeof ins1b / sizeof(int),
		   del1b, sizeof del1b / sizeof(int));
    }
    if (UnitNumber == 8)
    {
	// example replacing root and then removing it
	const int ins1b[] = {32,16,48,8,24,40,56,4,12,28,36,52,60,32};
	const int del1b[] = {32}; 
        unitDriver(ins1b, sizeof ins1b / sizeof(int),
		   del1b, sizeof del1b / sizeof(int));
    }
    if (UnitNumber == 9)
    {
	// example replacing a leaf and removing it
	const int ins1b[] = {32,16,48,8,24,40,56,4,12,28,36,52,60, 48};
	const int del1b[] = {48}; 
        unitDriver(ins1b, sizeof ins1b / sizeof(int),
		   del1b, sizeof del1b / sizeof(int));
    }
