...
/*
 * This function adds a user to the end of a list. It defaults all values to 
 * zero, for the sake of simplicity.
 * 
 * The input is the list you wish to add the new user to and the su_id of that
 * user.
 * 
 * There is no output as the list is passed by reference.
 */
void sas_add_tail(ListPtr list_ptr, int su_id)
{
    su_info_t *rec_ptr;
...
/*
 * This function will pass on the list and sort_type to the list_sort function
 * defined in list.c (individual values for sort-type can be found there). It 
 * also prints the amount of time it took to sort the list.
 * 
 * The input is the list that you wish to sort and the sorting type. Look to
 * list_sort for the values that sort_type accepts.
 * 
 * There is no output, as the list is pass by reference.
 */
