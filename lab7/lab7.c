 ...
 * 
 * Unit drivers
 *      -u 1    Tests normal use case. Adds items to a hash table, searches for
 *              existing and non existant entries and removes them.
 *      -u 2    Tests the code with a bad combination of hashing and table size
 *      -u 3    Deletion driver as in the homework set
 *
...
int main(int argc, char **argv)
{
    ...
    /* ----- unit drivers ----- */
    // Tests normal use case. Adds items to a hash table, searches for
    // existing and non existant entries and removes them.
    if (UnitTest == 1)
    {
        ...
        i[2] = 100;
        printf("\nInserting 3 keys\n");
        table_insert(T, 220, &(i[0]));
        table_insert(T, 330, &(i[1]));
        table_insert(T, 440, &(i[2]));
        if (Verbose)
            table_debug_print(T);

        assert(table_entries(T) == 3);
        assert(table_retrieve(T, 220) == &(i[0]));
        assert(table_retrieve(T, 330) == &(i[1]));
        assert(table_retrieve(T, 10) == NULL);
        assert(table_retrieve(T, 440) == &(i[2]));
        printf("\nPassed retreive test\n");

        printf("\nRemoving 3 keys\n");
        free(table_delete(T, 220));
        table_delete(T, 330);
        table_delete(T, 440);
        if (Verbose)
            table_debug_print(T);
    ...
    // Tests the code with a bad combination of hashing and table size
    else if (UnitTest == 2)
    {
        ...
        table_insert(T, 12, &(i[0]));
        table_insert(T, 48, &(i[1]));
        table_insert(T, 84, &(i[2]));
        printf("Performing final insertion. This should cause a crash!\n");
        if (Verbose)
            table_debug_print(T);
        table_insert(T, 120, &(i[3]));
    }
    else if (UnitTest == 3)
    {
        printf("----- Begin Deletion Driver -----\n");
    ...
    while ((c = getopt(argc, argv, "m:a:h:i:t:s:u:erbv")) != -1)
        switch(c) {
            ...
            default:
                      ...
                      printf("  -u 1|2|3  selects unit driver to run\n");
                      printf("  -u 3      selects the deletion unit driver\n");
                      exit(1);
        }
