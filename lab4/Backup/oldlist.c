 ...
 * linked list with an arbitrary datatype. For mp3, we have added a sorting
 * function to sort a list one of 4 possible ways.
 *
...
data_t * list_remove(list_t *list_ptr, list_node_t * idx_ptr)
{
    ...
    data_t *rtn;
    // insert your code after this line
    // Set the ptr to head if it's null
    if (idx_ptr == NULL)
        idx_ptr = list_ptr->head;

    if (list_ptr->current_list_size == 1)   // If there's only one item
        list_ptr->head = list_ptr->tail = NULL;
    else                                    // If there's more than one item
    {
        idx_ptr->prev->next = idx_ptr->next;
        idx_ptr->next->prev = idx_ptr->prev;
        if (list_ptr->tail == idx_ptr)
