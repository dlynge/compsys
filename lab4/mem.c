 ...
 * ECE 2230, Fall 2018
 * 
 * Purpose: This file is a dynamic memory manager and is able to 
 * keep track of free memory in the heap. This is accomplished by using
 * a 1-way linked list that points to each segment of available memory.
 * 
 * Assumptions: This file knows nothing about how the memory will
 * be used when given out. As a result, it is the user's responsibility
 * to free the memory once used.
 * 
 * Bugs: none known.
 */
...
{
    chunk_t *p = ((chunk_t *)return_ptr) - 1;
...
void *Mem_alloc(const int nbytes)
{
    ...
    // Move rover to spread allocation
    Rover = Rover->next;
    ...
    if (p == NULL) // If we don't find a match
    {
        ...
        if (mcRequest % PAGESIZE)
            mcRequest = ((mcRequest / PAGESIZE) + 1) * PAGESIZE;
...
void Mem_stats(void)
{
    ...
    int M = 0;
    int nItems = -1;
    int min = Dummy.next->size * sizeof(chunk_t);
    int max = min;
    
    do {
        nItems++;
        if (Rover->size * sizeof(chunk_t) > max)
            max = Rover->size * sizeof(chunk_t);
        if (Rover->size * sizeof(chunk_t) < min && Rover != &Dummy)
            min = Rover->size * sizeof(chunk_t);
        M += Rover->size * sizeof(chunk_t);
        Rover = Rover->next;
    } while (Rover != start_ptr);

    printf("Total items in free list -- %d\n", nItems);
    printf("Minimum size (bytes) in free list -- %d B\n", min);
    printf("Maximum size (bytes) in free list -- %d B\n", max);
    printf("Average size (bytes) in free list -- %d B\n", M / nItems);
    printf("Total memory (bytes) in free list -- %d B\n", M);
    printf("Number of calls to sbrk -- %d\n", NumSbrkCalls);
    printf("Number of pages requested -- %d\n", NumPages);
