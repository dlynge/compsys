 ...
 * -u 2      Tests 3 allocations and frees the two outside blocks first, creating
 *           a gap.
 * -u 3      Tests 3 allocations and frees as in test 2, but with sizes near
 *           PAGESIZE
 * -u 4      Tests the ability for the rover to spread allocations throughout the
 *           list by allocating and freeing one block before another.
 *
...
int main(int argc, char **argv)
{
    ...
    else if (dprms.UnitDriver == 2) // Testing for a gap
    {
        printf("\n----- Begin unit driver 2 -----\n");

        int unit_size = sizeof(chunk_t);
        int bytes_in_first_page = PAGESIZE;
        printf("There are %d bytes per page, and the size of chunk_t is %d bytes\n", 
                PAGESIZE, unit_size); 
        ...
        // ----- ALLOCATION ----- 
        // Allocate three blocks of memory taking up a page
        num_bytes_1 = bytes_in_first_page / 2;
        bytes_in_first_page -= num_bytes_1;
        p1 = Mem_alloc(num_bytes_1 - unit_size);

        num_bytes_2 = bytes_in_first_page / 2;
        bytes_in_first_page -= num_bytes_2;
        p2 = Mem_alloc(num_bytes_2 - unit_size);

        num_bytes_3 = bytes_in_first_page;
        bytes_in_first_page -= num_bytes_3;
        p3 = Mem_alloc(num_bytes_3 - unit_size);

        assert(bytes_in_first_page == 0);

        printf("Allocated 3 memory blocks taking up an entire page\n");
        Mem_print();
        printf("Free list above should be empty\n\n");
        ...
        printf("Free'd memory blocks 1 and 3\n");
        Mem_print();
        printf("Free list above should show a gap\n\n");

        // Free last block
        Mem_free(p2);
        p2 = NULL;
        printf("Free'd last block 2\n");
        Mem_print();
        printf("Free list above should be a page in size\n\n");

        Mem_stats();

        printf("\n----- End unit test driver 2 -----\n");
    }
    else if (dprms.UnitDriver == 3) // Testing for a whole page and some change
    {
        printf("\n----- Begin unit driver 3 -----\n");
        ...
        num_bytes_2 = PAGESIZE - 1;
        p2 = Mem_alloc(num_bytes_2 - unit_size);

        num_bytes_3 = PAGESIZE + 1;
        p3 = Mem_alloc(num_bytes_3 - unit_size);


        printf("Allocated 3 memory blocks, each around a page in size\n");
        Mem_print();
        printf("Free list above should show 1 empty item\n\n");
    ...
    else if (dprms.UnitDriver == 4) // Testing for Rover spreading allocation
    {
        printf("\n----- Begin unit driver 4 -----\n");
        ...
        num_bytes_1 = 256;
        p1 = Mem_alloc(num_bytes_1 - unit_size);
        printf("Allocated 1 memory block of size 256 B\n");
        Mem_print();
        printf("Free list above should show 1 empty item\n\n");

        // Free the first section
        Mem_free(p1);
        p1 = NULL;
        printf("Free'd the previous memory block\n");
        Mem_print();
        printf("Free list above should have the size of one page\n\n");

        // Allocate new section
        num_bytes_2 = 128;
        p2 = Mem_alloc(num_bytes_2 - unit_size);
        printf("Allocated a new block of size 128 B\n");
        Mem_print();
        printf("Free list above should show how the allocation rover moves\n\n");
